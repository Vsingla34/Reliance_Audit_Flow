...
export function DistributorsModule() {
  ...
  useEffect(() => {
    ...
    // Subscribe to realtime changes on the distributors table
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEditingDist(null);
      setFormData({ code: '', name: '', approvedValue: 0, aseId: '', active: true });
    } catch (error) {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const newDistributors = lines.map(line => {
          ...
          // Try to map ASE by email from existing users
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex gap-4">
          ...
          <button 
            onClick={() => { setEditingDist(null); setFormData({ code: '', name: '', approvedValue: 0, aseId: '', active: true }); setIsModalOpen(true); }}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <select className="w-full px-4 py-3 bg-zinc-50 border-none rounded-xl focus:ring-2 focus:ring-black transition-all" value={formData.aseId} onChange={(e) => setFormData({ ...formData, aseId: e.target.value })}>
                          <option value="">Select an ASE...</option>
                          {users.filter(u => u.role === 'ase').map(user => (
                            <option key={user.uid} value={user.uid}>{user.name} ({user.region || 'No Region'})</option>
                          ))}
                        </select>
                      </div>
                    </div>
