...
export function SchedulerModule() {
  ...
  const approveAndSchedule = async (proposalDate: string) => {
    if (!negotiationTicket || !approvalAuditorId) {
      alert("Please select an Auditor from the dropdown first!");
      return;
    }
          ...
          <div className="p-4 md:p-6 border-b border-zinc-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-zinc-50/50">
            <div className="flex items-center gap-4">
              <div className="flex bg-zinc-200/50 p-1 rounded-xl">
                <button
                  onClick={() => setPendingTab('approval')}
                  className={cn(
                    "px-4 py-2 text-sm font-bold rounded-lg transition-all flex items-center gap-2", 
                    pendingTab === 'approval' ? "bg-white text-amber-600 shadow-sm" : "text-zinc-500 hover:text-black"
                  )}
                >
                ...
                <button
                  ...
                  className={cn(
                    "px-4 py-2 text-sm font-bold rounded-lg transition-all flex items-center gap-2", 
                    pendingTab === 'waiting' ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-black"
                  )}
                >
          ...
          <div className="p-4 max-h-[360px] overflow-y-auto custom-scrollbar bg-zinc-50/30">
            {displayTickets.length === 0 ? (
              <div className="text-center py-8 text-zinc-400">
                ...
                <p className="text-sm font-medium">No tickets in this folder.</p>
              </div>
            ) : (
                ...
                {displayTickets.map(ticket => {
                  ...
                  return (
                    <motion.div 
                      layout
                      key={ticket.id}
                      onClick={() => setNegotiationTicket(ticket)}
                      className={cn(
                        "bg-white p-4 rounded-2xl border shadow-sm hover:shadow-md transition-all cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-4 group",
                        pendingTab === 'approval' ? "border-amber-200 hover:border-amber-400" : "border-zinc-200 hover:border-zinc-400"
                      )}
                    >
                      <div className="flex items-center gap-4 min-w-0">
                        <div className={cn(
                          "w-10 h-10 rounded-xl flex items-center justify-center shrink-0",
                          pendingTab === 'approval' ? "bg-amber-50 text-amber-600" : "bg-zinc-100 text-zinc-500"
                        )}>
                          <Store size={18} />
                        </div>
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-[10px] text-zinc-500 bg-zinc-100 px-2 py-0.5 rounded font-medium truncate max-w-[120px]">
                              ASE: {ase?.name || 'Unassigned'}
                            </span>
                      ...
                      <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto">
                        {pendingTab === 'approval' && lastProposal && (
                          <div className="text-left md:text-right">
                            <p className="text-[10px] font-bold text-amber-600 uppercase tracking-wider mb-0.5 flex items-center gap-1">
                              <MessageSquare size={10} /> Latest Reply
                            </p>
                            <p className="text-xs font-medium text-zinc-700 truncate max-w-[200px]">"{lastProposal.remarks || 'No remarks'}"</p>
                          </div>
                        )}

                        <button className="px-4 py-2 bg-zinc-100 text-zinc-900 text-xs font-bold rounded-xl group-hover:bg-black group-hover:text-white transition-colors whitespace-nowrap">
                          Review
                        </button>
                      </div>
                    </motion.div>
          ...
          {days.map(day => {
            ...
            return (
              <div 
                key={day.toISOString()} 
                className={cn(
                  "min-h-[120px] rounded-2xl p-3 border transition-colors flex flex-col gap-2",
                ...
                <div className="space-y-2 flex-1 overflow-y-auto custom-scrollbar pr-1">
                  {dayTickets.map(ticket => {
                    ...
                    return (
                      <div 
                        key={ticket.id}
                        onClick={() => ticket.status === 'tentative' ? setNegotiationTicket(ticket) : null}
                        className={cn(
                          ...
                          "bg-emerald-50 border-emerald-100/50 cursor-default"
                        )}
                        title={ticket.status === 'tentative' ? 'Click to negotiate dates' : 'Scheduled'}
                      >
              ...
              <div className="p-6 md:p-8 overflow-y-auto bg-zinc-50/50 flex-1 space-y-6 custom-scrollbar">
                {(!negotiationTicket.dateProposals || negotiationTicket.dateProposals.length === 0) ? (
                  <div className="text-center py-8 text-zinc-400">
                    ...
                    <p className="text-xs">Be the first to suggest an audit date or leave a remark below.</p>
                  </div>
                ) : (
                  negotiationTicket.dateProposals.map((prop) => (
                    <div key={prop.id} className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 text-xs">
                              ...
                              <span className="text-zinc-400">{prop.email}</span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="pl-11 space-y-3">
                        {prop.date && (
                          <div className="inline-flex items-center gap-2 bg-amber-50 text-amber-700 px-3 py-1.5 rounded-lg border border-amber-100 text-sm font-bold">
                            <CalendarIcon size={14} /> Proposed: {format(new Date(prop.date), 'dd MMM yyyy')}
                          </div>
                        )}
