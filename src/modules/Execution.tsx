      ...
      <div className="space-y-4 sm:space-y-6 pb-12 w-full min-w-0">
        ...
        <div className="bg-white rounded-[1.5rem] sm:rounded-[2.5rem] p-5 sm:p-8 border border-zinc-200 shadow-sm w-full">
          ...
          {(activeTicket.fieldAuditors?.length || (!isSubmitted && activeTicket.status !== 'drainage_pending' && (isAuditor || isAdminOrHO))) ? (
            <div className="mb-6 sm:mb-8 p-4 sm:p-6 bg-zinc-50 border border-zinc-200 rounded-[1.5rem] sm:rounded-[2rem] shadow-sm">
              <h4 className="font-bold text-base sm:text-lg mb-4 flex items-center gap-2"><UserIcon className="text-blue-500" size={18} /> Field Auditor Details</h4>
              
              {!isSubmitted && activeTicket.status !== 'drainage_pending' && (isAuditor || isAdminOrHO) && (
                <form onSubmit={handleAddFieldAuditor} className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-4">
                  ...
                  <button type="submit" className="w-full sm:w-auto px-6 py-3 bg-black text-white font-bold rounded-xl hover:bg-zinc-800 transition-colors shadow-md active:scale-95 text-sm">Add Detail</button>
                </form>
              )}
              ...
              {activeTicket.fieldAuditors && activeTicket.fieldAuditors.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {activeTicket.fieldAuditors.map((fa, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3 sm:p-4 bg-white border border-zinc-200 shadow-sm rounded-xl sm:rounded-2xl">
                      <div>
                        <p className="font-bold text-zinc-900 text-sm">{fa.name}</p>
                        <p className="text-[10px] sm:text-xs text-zinc-500 font-medium mt-0.5">{fa.phone}</p>
                      </div>
                      {!isSubmitted && activeTicket.status !== 'drainage_pending' && (isAuditor || isAdminOrHO) && (
                ...
                <p className="text-xs sm:text-sm text-zinc-500 italic">No field auditors added yet.</p>
              )}
            </div>
          ) : null}
